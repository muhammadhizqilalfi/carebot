...
export default function FacilitiesPage() {
  ...
  const facilityTypes = [
    "Semua",
    ...Array.from(new Set(mockFacilities.map((facility) => facility.type))),
  ];
  ...
  const filteredFacilities = mockFacilities.filter((facility) => {
    ...
    const matchesFilter =
      activeFilter === "Semua" || facility.type === activeFilter;
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm text-slate-500 leading-7">
              Explore trusted clinics, BPJS providers, and pharmacies in Banda
              Aceh with a modern map view.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-100px)]">
            ...
            {filteredFacilities.length === 0 ? (
              ...
              <div className="space-y-5">
                {filteredFacilities.map((facility) => (
                  <div
                    key={facility.id}
                    className="rounded-[32px] bg-white border border-slate-100 p-5 shadow-sm hover:shadow-md transition-shadow"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-sm uppercase tracking-[0.24em] text-slate-400 font-bold">
                          {facility.type}
                        </p>
                        <h3 className="mt-3 text-xl font-bold text-slate-900 leading-tight">
                          {facility.name}
                        </h3>
                      </div>
          ...
          <section className="lg:col-span-8 rounded-[32px] overflow-hidden bg-white shadow-[0_18px_40px_rgba(15,23,42,0.08)] border border-slate-100 h-full min-h-[640px]">
            <MapComponent
              ...
              selectedFacility={selectedFacility} // Kirim state ini ke Map
            />
          </section>
        </div>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-slate-500">
                        ({selectedFacility.reviewCount} ulasan)
                      </span>
                    </div>
                  </div>
